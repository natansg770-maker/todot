...
async function main() {
  ...
  const byNameOld = new Map<string, typeof source.people>();
  for (const p of source.people) {
    const list = byNameOld.get(p.name) || [];
    list.push(p);
    byNameOld.set(p.name, list);
  }
  const byNameNew = new Map<string, typeof fresh.people>();
  for (const p of fresh.people) {
  ...
  const unmatchedOld: { name: string; missing: number }[] = [];
  ...
  let mapped = 0;
  let skipped = 0;
  const assignments: typeof source.assignments = [];
  ...
  const claimRequests = (source.claimRequests || [])
    ...
    .filter(
      (r: {
        ...
        targetAssigneeId?: string;
        recipientId?: string;
      }) => r.requesterId && r.targetAssigneeId && r.recipientId,
    );
  ...
  const nathan = restored.people.find((p) =>
    p.name.includes("נתן שמחה גרינברג"),
  );
  console.log({
    ...
    nathanId: nathan?.id,
    nathanAssignments: restored.assignments.filter(
      (a) => a.assigneeId === nathan?.id,
    ).length,
  });
}
